...
const SingleProductSchema = z.object({
    ...
    weightVariants: z.array(z.object({
        ...
        price: z.string().min(1, "Regular price is required")
            .refine((val) => !isNaN(Number(val)) && Number(val) > 0, {
                message: "Regular price must be a valid positive number"
            }),
        mrp: z.string().min(1, "MRP price is required")
            .refine((val) => !isNaN(Number(val)) && Number(val) > 0, {
        ...
        if (variant.startSaleOn instanceof Date && variant.endSaleOn instanceof Date) {
